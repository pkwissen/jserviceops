...
# ---------- low level helpers ----------
def _read_all_sheets(path: Optional[Union[str, io.BytesIO]]) -> Dict[str, pd.DataFrame]:
    """Read all sheets from a filesystem path or a file-like BytesIO.
    Return dict of sheet_name -> DataFrame. If source doesn't exist or is None return empty dict.
    """
    if path is None:
        return {}
    # file-like (BytesIO) -> use directly
    try:
...
def _write_all_sheets(path: Union[str, io.BytesIO], sheets: Dict[str, pd.DataFrame]) -> None:
    """Write the provided sheets dict to a filesystem path or a file-like BytesIO.
    If `path` is a BytesIO the workbook will be written into that stream and the stream
    will be seeked to 0 on return.
    """
    ...
    if hasattr(path, "write"):
        ...
        try:
            ...
            pass
        try:
            # Ensure we truncate existing BytesIO content so new workbook
            # overwrites previous bytes (prevents corruption when size changes)
            path.truncate(0)
            path.seek(0)
        except Exception:
            pass
...
# ---------- public API ----------
def ensure_user_sheets(path: Optional[Union[str, io.BytesIO]]) -> None:
    ...
    for sheet in USER_SHEETS:
        if sheet not in sheets:
            ...
            if df is None or df.shape[1] == 0:
                ...
                # No action required.
    # If path is a file-like we write back into the stream; if path is a filesystem path we overwrite the file.
    if changed:
...
def get_users(excel_path: Optional[Union[str, io.BytesIO]], sheet: str) -> List[Tuple[str, str]]:
    """
...
def add_user(excel_path: Optional[Union[str, io.BytesIO]], sheet: str, name: str, email: str) -> Tuple[bool, Optional[io.BytesIO]]:
    """
    ...
    # If excel_path is a file-like stream, write into it and return the stream so callers can upload.
    if hasattr(excel_path or '', "write"):
...
def remove_user(excel_path: Optional[Union[str, io.BytesIO]], sheet: str, name: str, email: str) -> Tuple[bool, Optional[io.BytesIO]]:
    """
