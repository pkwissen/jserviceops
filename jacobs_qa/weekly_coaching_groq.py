...
def _fallback_classify(text: str) -> str:
    ...
    for keys, label in _keyword_map:
        if any(k in T for k in keys):
            return label
    return ""  # unknown → let it be blank (matches your rules)

def categorize_quality_parameter(areas_text: str) -> str:
    """Classify 'Areas to Improve' into a single known category or propose a new one (Groq single-call; fallback keywords)."""
    if not areas_text or str(areas_text).strip() == "":
        return ""

    system = "You are a strict text classifier. Return exactly ONE short category name, or empty string if no improvement needed."
    prompt = f"""
KNOWN CATEGORIES:
{', '.join(QUALITY_CATEGORIES)}

RULES:
- Return ONLY ONE label. Never return multiple labels.
- If the text clearly belongs to one of the known categories, return ONLY that category.
- If the text does not fit any known category, return a NEW short category name (max 4 words).
- If the text means no improvement needed (e.g., "satisfactory", "done well", "nothing to improve"), return "" (empty string).
- Output must be only the category name or "" (no explanations, no punctuation).
...
{areas_text}
    ...
    if result:
        ...
        # Guard against the model echoing rules
        if len(cleaned) > 60 or ":" in cleaned:
            # probably not a clean label → fallback
